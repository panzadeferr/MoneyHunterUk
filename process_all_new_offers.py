...
# Find the next function definition after get_manual_offers
next_func_pattern = r'\n\s*def \w+'
next_func_match = re.search(next_func_pattern, content[start_pos + 50:])
if not next_func_match:
    print("Could not find next function")
    exit()

# The get_manual_offers function ends before the next function
func_end = start_pos + 50 + next_func_match.start()

# Extract the function content
func_content = content[start_pos:func_end]

print(f"Function starts at: {start_pos}")
print(f"Function ends at: {func_end}")
print(f"Function length: {func_end - start_pos} chars")

# Create the complete new function with all 60 offers
new_function = '''def get_manual_offers() -> List[Dict]:
...
# Add the 60 new offers
new_offers_text = '''        # NEW OFFERS (60 total)
        {"store": "First Direct", "item": "Switch Account — £175 Cash", "deal_price": "£175", "link": "https://www.firstdirect.com/banking/switch/", "original_price": "£0", "saving_percent": 100, "type": "bank_switch", "category": "bank", "code": "", "expires": "2026