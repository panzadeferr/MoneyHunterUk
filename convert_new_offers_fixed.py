#!/usr/bin/env python3
"""
Script to convert new_offers.json format to scraper.py manual offer format
"""
...
def convert_offer(new_offer: Dict) -> Dict:
    """Convert a new offer to scraper.py format"""
    # Map the fields
    converted = {
        "store": new_offer["store"],
        "item": new_offer["item"],
        "deal_price": new_offer["deal_price"],
        "link": new_offer["link"],
        "original_price": new_offer["original_price"],
        "saving_percent": new_offer["saving_percent"],
        "type": new_offer["type"],
        "category": new_offer["category"],
        "code": new_offer["code"],
        "steps": new_offer["steps"],
        "timeFrame": f"Expires {new_offer['expires']}" if new_offer.get("expires") else "Varies"
    }
    
    # Remove empty fields
    if not converted["code"]:
        del converted["code"]
    
    return converted

def main():
    # Load the new offers from the provided JSON
    # The user provided the JSON content directly, so we'll use it
    new_offers_json = """[
  {
    "store": "First Direct",
